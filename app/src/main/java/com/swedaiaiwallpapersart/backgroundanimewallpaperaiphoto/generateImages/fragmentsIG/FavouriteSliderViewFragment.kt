...
class FavouriteSliderViewFragment : Fragment() {
    private lateinit var binding: FragmentCreationSliderViewBinding
    private var myContext:Context? = null
    private var arrayList = ArrayList<FavouriteListIGEntity>()
    ...
    private fun onCreatingViewCalling() {
        ...
        if (arrayListJson != null && pos != null) {
            val gson = Gson()
            val arrayListType = object : TypeToken<ArrayList<FavouriteListIGEntity>>() {}.type
            arrayList = gson.fromJson(arrayListJson, arrayListType)
            position = pos
        }
        val myId = arrayList[position].id
        val prompt = arrayList[position].prompt
        functionality(myId,prompt)
    }
    ...
    private fun functionality(myId: Int?, prompt: String?) {
        ...
        getLargImage = arrayList[position].image!!
...
//            })
    ...
    private fun setViewPager() {
        val adopter = FavouriteWallpaperSliderAdapter(arrayList, viewPager2!!)
        ...
        val viewPagerChangeCallback = object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(positi: Int) {
                getLargImage = arrayList[positi].image!!
                position = positi
                getBitmapFromGlide(getLargImage)
                checkAlreadyFavourite()
